...
The task is set out in the [Take-home Exercise 1 brief](https://isss626-ay2026-27aug.netlify.app/take-home_ex01). The executive summary slides are [here](Take-home_Ex01_slides.html).
...
SP data ends on 30 June, and the NRT data after it is provisional. NRT data also has no `type` flag, so fixed heat sources cannot be filtered out. [Two further checks](#sec-static), under Merging Repeated Detections, looks for them instead.
:::
...
kalimantan %>% st_drop_geometry() %>% count(NAME_1, name = "units") %>% kable()
```
...
No cell burned on more than `r max(days_per_cell$days)` of the 84 days, so there is no fixed heat source. About `r round(100 * mean(fires_all$scan > 2))` percent of events come from pixels wider than 2 km, where one fire can light up two cells on the same day. The space-time test therefore treats same-day pairs separately.
...
kable(prep_log, format.args = list(big.mark = ","))
```
...
reburn_tab %>% select(-within_week) %>% kable(col.names = c("days since the cell last burned", "fire-days"))
```
...
st_stack <- do.call(c, lapply(snap_days, function(d) to_rast(fire_stkde$z[[as.character(d)]] * n_ev)))
names(st_stack) <- snap_lab
```

Values are expected fire-days per km² per day, so all six maps share one scale.
...
```{r}
#| label: fig-stkde
#| fig-cap: "Spatio-temporal kernel density on six days, expected fire-days per km² per day. Data from NASA FIRMS MODIS C6.1"
#| fig-height: 11
#| echo: false
tm_shape(st_stack) +
  tm_raster(col.scale = tm_scale_intervals(style = "fixed",
...
In mid July and early August the main warm area is the neck. By 26 August the north and the south are warming, and by 18 September the south-central peat peaks at about `r round(st_peak, 2)` fire-days per km² per day, with a second hot spot on the east edge.
...
**Parameters.** Sigma is `r round(sig_scott, 1)` km with leave-one-out, and the intensity is estimated again for each simulation (Baddeley et al., 2015).
...
VIIRS rows with low confidence (`l`) are removed, and the detection with the highest fire radiative power is kept for each cell and day.
...
tibble(model = c("constant intensity", "access model"),
       AIC = round(c(AIC(m_null), AIC(m_access)))) %>% kable()
```
...
kable(strict_sum, digits = 3)
```

`land_covered` is the zone's average share of the regency, weighted by new cells per day. The test starts on 4 July and covers `r format(watch_sum$newly_burning, big.mark = ",")` newly burning cells (`r sum(fires$is_new) - watch_sum$newly_burning` fewer than the season total).
