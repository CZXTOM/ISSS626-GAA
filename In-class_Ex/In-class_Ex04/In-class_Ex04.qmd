...
`left_join()` of **dplyr** attaches the indicators to the polygons by the shared `County` column. `select()` keeps NAME_2, ID_3, NAME_3, County, GDPPC, GIO, Agri and Service. After the join these are at positions 1 to 3, 7, 15, 16, 31 and 32.
...
::: {.callout-note}
## Learning diary
Both approaches give the same answer, `r bw_CV` nearest neighbours for CV and `r bw_AIC` for AICc. So each county's local statistics use about a quarter of the province. `longlat = T` is needed because the data is still in WGS 84. So distances have to be great circle distances in kilometres.
:::
...
`cbind()` then adds those columns to *hunan_sf*. The rows are in the same county order, so no join key is needed.
